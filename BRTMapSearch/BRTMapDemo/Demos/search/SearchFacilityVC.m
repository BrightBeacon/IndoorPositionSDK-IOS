...
- (IBAction)categoryButtonClicked:(id)sender {
    ...
    NSArray *facilities = self.mapView.getAllFacilityOnCurrentFloor;
    for (BRTPoi *poi in facilities) {
        NSString *key = [NSString stringWithFormat:@"%ld(%@)",poi.categoryID,[kCatergoryDic objectForKey:@(poi.categoryID).stringValue]];
        NSMutableArray *marr = [_data valueForKey:key];
        if (!marr) {
            marr = [NSMutableArray array];
            [_data setObject:marr forKey:key];
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if(buttonIndex != actionSheet.cancelButtonIndex){
        ...
        NSString *cid = [actionSheet buttonTitleAtIndex:buttonIndex];
        NSMutableArray *marr = [NSMutableArray array];
        for (BRTPoi *poi in _data[cid]) {
            MGLPointAnnotation *ann = [[MGLPointAnnotation alloc] init];
            ann.coordinate = poi.labelPoint.coordinate;
            ann.title = poi.name;
            [marr addObject:ann];
        }
        [self.mapView addAnnotations:marr];
    }
}
